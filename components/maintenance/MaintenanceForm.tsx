...
export default function MaintenanceForm({ bookingId, onSubmitted }: { bookingId: string; onSubmitted?: () => void }) {
  ...
  const PRIORITIES = [
    { value: "low", label: t("priorities.low"), color: "bg-zinc-100 text-zinc-600" },
    { value: "medium", label: t("priorities.medium"), color: "bg-blue-100 text-blue-600" },
    { value: "high", label: t("priorities.high"), color: "bg-amber-100 text-amber-700" },
    { value: "urgent", label: t("priorities.urgent"), color: "bg-red-100 text-red-600" },
  ];
    ...
    <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 overflow-hidden">
      <button onClick={() => setExpanded(v => !v)}
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-zinc-900 dark:text-white">{t("reportIssue")}</span>
        </div>
        ...
        {expanded && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
            ...
            <div className="px-5 pb-5 space-y-4">
              <div>
                <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5 block">{t("issueTitle")}</label>
              ...
              <div>
                <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5 block">{t("description")}</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5 block">{t("category")}</label>
                  <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
                    ...
                    {CATEGORIES.map(c => <option key={c} value={c} className="capitalize">{t(`categories.${c}`)}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5 block">{t("priority")}</label>
