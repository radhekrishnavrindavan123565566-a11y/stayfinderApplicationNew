...
export default function Footer() {
  ...
  const EXPLORE = [
    ...
    { label: t("roommates"),  href: "/roommates" },
    { label: t("becomeHost"), href: "/auth/register" },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 lg:py-16">
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}
          ...
          <motion.div variants={fadeUp} className="sm:col-span-2 lg:col-span-2">
            ...
            <p className="text-sm leading-relaxed max-w-xs mb-5 text-zinc-400">{t("tagline")}</p>
            <div className="flex gap-3">
              {SOCIALS.map(({ Icon, label, href }) => (
                <motion.a key={label} href={href} target="_blank" rel="noopener noreferrer"
                  whileHover={{ scale: 1.12, backgroundColor: "#f43f5e" }} whileTap={{ scale: 0.95 }}
          ...
          <motion.div variants={fadeUp}>
            <h4 className="text-white font-semibold mb-4 text-sm tracking-wide">{t("explore")}</h4>
          ...
          {/* Support */}
          <motion.div variants={fadeUp}>
            <h4 className="text-white font-semibold mb-4 text-sm tracking-wide">{t("support")}</h4>
            <ul className="space-y-3 text-sm">
              {SUPPORT.map((item) => (
                <li key={item.label}>
                  <Link href={item.href} className="hover:text-white transition-colors flex items-center gap-1.5 group py-0.5">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.3 }}
          ...
          <p suppressHydrationWarning>© {new Date().getFullYear()} Nestora. {t("allRights")}</p>
          <div className="flex gap-4 sm:gap-6">
            <Link href="/terms"   className="hover:text-white transition-colors py-1">{t("privacy")}</Link>
            <Link href="/terms"   className="hover:text-white transition-colors py-1">{t("terms")}</Link>
            <Link href="/contact" className="hover:text-white transition-colors py-1">{t("contactUs")}</Link>
          </div>
        </motion.div>
      </div>
    </footer>
