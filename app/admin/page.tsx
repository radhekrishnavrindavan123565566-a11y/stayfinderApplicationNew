...
export default function AdminPage() {
  ...
  const statCards = [
    ...
    { icon: <DollarSign className="w-5 h-5" />, label: "Total Revenue", value: stats ? `₹${(stats.revenue || 0).toLocaleString()}` : "-", color: "bg-amber-50 text-amber-500 dark:bg-amber-950/30 dark:text-amber-400" },
    { icon: <TrendingUp className="w-5 h-5" />, label: "Platform Fees", value: stats ? `₹${(stats.platformRevenue || 0).toLocaleString()}` : "-", color: "bg-purple-50 text-purple-500 dark:bg-purple-950/30 dark:text-purple-400" },
                    ...
                    {stats?.recentBookings && stats.recentBookings.length > 0 ? (
                      stats.recentBookings.map((b: { 
                        ...
                        >
                          ...
                          <span className="font-semibold text-zinc-900 dark:text-white ml-4 flex-shrink-0">₹{(b.totalPrice || 0).toLocaleString("en-IN")}</span>
                        </motion.div>
                      ))
                    ) : (
...
function BulkMarketingPanel({ authHeaders }: { authHeaders: () => { headers: { Authorization: string } | { Authorization?: undefined } } }) {
  const [title, setTitle] = useState("SST Home Solutions — Special Offer for Lucknow!");
  const [message, setMessage] = useState("Namaskar! SST Home Solutions par aaj hi apna ghar dhundein ya list karein. Lucknow ke sabse verified listings sirf hamare paas. Visit: ssthomesolutions.com");
